...
def ullmann(used_columns, current_row, M, G_matrix, P_matrix, isomorphisms, no_calls=0):
    no_calls += 1
    ...
    for c in range(M.size()[1]):
        if not used_columns[c]:
            ...
            M[current_row][c] = 1
            no_calls = ullmann(used_columns, current_row + 1, M, G_matrix, P_matrix, isomorphisms, no_calls)
            used_columns[c] = False
    
    return no_calls

def ullmann_v2(used_columns, current_row, M, G_matrix, P_matrix, isomorphisms, no_calls):
    no_calls += 1
    ...
    for c in range(M.size()[1]):
        if not used_columns[c] and M[current_row][c] == 1:
            ...
            no_calls = ullmann_v2(used_columns, current_row + 1, M_copy, G_matrix, P_matrix, isomorphisms, no_calls)
            used_columns[c] = False
...
def main():
    ...
    mat = Matrix((len(unique_P_list), len(unique_G_list)))
    used_columns = [False for _ in range(len(unique_G))]
    isomorphisms = []
    no_calls_1 = ullmann(used_columns, 0, mat, Matrix_G, Matrix_P, isomorphisms, 0)
    print(f"v1.0 - Izomorfizmy: {len(isomorphisms)}, Wywolania: {no_calls_1}")
    ...
    no_calls_2 = ullmann_v2(used_columns, 0, M0, Matrix_G, Matrix_P, isomorphisms_v2, 0)
    print(f"v2.0 - Izomorfizmy: {len(isomorphisms_v2)}, Wywolania: {no_calls_2}")
